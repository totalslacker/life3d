...
struct GridModel: Sendable {
    ...
    mutating func loadHeartSurface() {
        clearAll()
        let n = size
        let half = Float(n) / 2.0
        let scale: Float = 1.8 / half
        ...
        for ix in 0..<n {
            for iy in 0..<n {
                for iz in 0..<n {
                    ...
                    let x2 = x * x, y2 = y * y, z2 = z * z
                    let z3 = z2 * z
                    let inner = x2 + 2.25 * y2 + z2 - 1.0
                    let value = inner * inner * inner - x2 * z3 - 0.1125 * y2 * z3
                    if abs(value) < threshold {
                        setCell(x: ix, y: iy, z: iz, alive: true)
                    }
                }
    ...
    mutating func loadNeoviusSurface() {
        clearAll()
        let n = size
        let half = Float(n) / 2.0
        // Scale so ~1.5 periods fit in the grid
        let scale: Float = (3.0 * .pi) / half
        ...
        for ix in 0..<n {
            for iy in 0..<n {
                for iz in 0..<n {
                    ...
                    let value = 3.0 * (cx + cy + cz) + 4.0 * cx * cy * cz
                    if abs(value) < threshold {
                        setCell(x: ix, y: iy, z: iz, alive: true)
                    }
                }
    ...
    mutating func loadChmutovSurface() {
        clearAll()
        let n = size
        let half = Float(n) / 2.0
        let scale: Float = 2.2 / half
        ...
        for ix in 0..<n {
            for iy in 0..<n {
                for iz in 0..<n {
                    ...
                    let y = (Float(iy) - half + 0.5) * scale
                    let z = (Float(iz) - half + 0.5) * scale
                    // T₄(t) = 8t⁴ - 8t² + 1 (Chebyshev polynomial of the first kind, degree 4)
                    let tx = 8*x*x*x*x - 8*x*x + 1
                    let ty = 8*y*y*y*y - 8*y*y + 1
                    let tz = 8*z*z*z*z - 8*z*z + 1
                    let value = tx + ty + tz
                    if abs(value) < threshold {
                        setCell(x: ix, y: iy, z: iz, alive: true)
                    }
    ...
    mutating func loadPiriformSurface() {
        clearAll()
        let n = size
        let half = Float(n) / 2.0
    ...
    mutating func loadPillowSurface() {
        clearAll()
        let n = size
        let half = Float(n) / 2.0
        let scale: Float = 2.0 / half
        ...
        for ix in 0..<n {
            for iy in 0..<n {
                for iz in 0..<n {
                    ...
                    let y2 = y * y
                    let z2 = z * z
                    // Pillow surface: x²z² + y²z² + x²y² - x²y²z² = k
                    // Creates a cushion/pillow shape with concave faces along axes
                    let value = x2 * z2 + y2 * z2 + x2 * y2 - x2 * y2 * z2 - 0.6
                    if abs(value) < threshold {
                        setCell(x: ix, y: iy, z: iz, alive: true)
                    }
                }
    ...
    mutating func loadEightSurface() {
        clearAll()
        let n = size
        let half = Float(n) / 2.0
        let scale: Float = 2.0 / half
        ...
        for ix in 0..<n {
            for iy in 0..<n {
                for iz in 0..<n {
                    ...
                    let y = (Float(iy) - half + 0.5) * scale
                    let z = (Float(iz) - half + 0.5) * scale
                    let value = sin(2.0 * pi * x) * sin(pi * y) + sin(2.0 * pi * y) * sin(pi * z) + sin(2.0 * pi * z) * sin(pi * x)
                    if abs(value) < threshold {
                        setCell(x: ix, y: iy, z: iz, alive: true)
                    }
                }
    ...
    mutating func loadLoxodrome() {
        ...
        let n = size
        let half = Float(n) / 2.0
        let scale = Float(n) * 0.42
        let R: Float = 0.90          // sphere radius
        let thickness: Float = 1.4   // tube thickness in cells
        let windingRate: Float = 4.0 // controls how tightly the spiral winds

        // Loxodrome: a curve on a sphere that crosses every meridian at a constant angle.
        // Parametric form: x = R·cos(t)·cos(a·t), y = R·cos(t)·sin(a·t), z = R·sin(t)
        // where t ∈ (-π/2, π/2) is latitude and a is the winding rate.

        // Rasterize by dense point sampling along the curve
        let steps = n * n * 8
        var curvePoints: [SIMD3<Float>] = []
        curvePoints.reserveCapacity(steps)
        for i in 0..<steps {
            let t = Float.pi * (Float(i) / Float(steps - 1) - 0.5) * 0.95 // avoid poles
            let cosT = cosf(t)
            let sinT = sinf(t)
            let phi = windingRate * t
            let px = R * cosT * cosf(phi)
            let py = R * cosT * sinf(phi)
            let pz = R * sinT
            curvePoints.append(SIMD3<Float>(px, py, pz))
        }

        // Activate cells near curve points
        for x in 0..<n {
            for y in 0..<n {
                for z in 0..<n {
                    let fx = (Float(x) - half) / scale
                    ...
                    let threshSq = (thickness / scale) * (thickness / scale)
    ...
    mutating func loadAstroid() {
        ...
        // Astroid (4-cusped hypocycloid): x = R*cos^3(t), y = R*sin^3(t)
        ...
        for rs in 0..<revolutionSteps {
            ...
            for ps in 0..<profileSamples {
                ...
                let cosT = cos(t)
                let sinT = sin(t)
                // Astroid profile in XY plane
                let px = scale * cosT * cosT * cosT
                let py = scale * sinT * sinT * sinT
    ...
    mutating func loadEpitrochoid() {
        ...
        // R = fixed circle radius, r = rolling circle radius, d = tracing point distance
        // Using R=3, r=1, d=0.5 for a 3-lobed spirograph pattern
        let bigR: Float = 3.0
        let smallR: Float = 1.0
        let d: Float = 0.5
    ...
    mutating func loadWitchOfAgnesi() {
        clearAll()
        let n = size
        let half = Float(n) / 2.0
        let thickness: Float = max(1.0, Float(n) / 12.0)
        ...
        // Witch of Agnesi: y = a³ / (x² + a²)
        // Bell-shaped curve, symmetric about y-axis
        // a controls the width/height; using a = 1.0
        let a: Float = 1.0
        let xRange: Float = 3.0  // sample x from -3 to 3
        ...
        for rs in 0..<revolutionSteps {
            ...
            for ps in 0..<profileSamples {
                let t = Float(ps) / Float(profileSamples) * 2.0 * xRange - xRange
                let yVal = (a * a * a) / (t * t + a * a)
                let px = t * scale / xRange
                let py = yVal * scale - half * 0.3
                // Revolve around Y axis
                let wx = px * cosPhi + half
                let wy = py + half
                let wz = px * sinPhi + half
    ...
    mutating func loadFoliumOfDescartes() {
        clearAll()
        let n = size
        let half = Float(n) / 2.0
        let thickness: Float = max(1.0, Float(n) / 12.0)
        ...
        // Parametric form: x = 3at/(1+t³), y = 3at²/(1+t³)
        // The curve has a distinctive leaf (folium) shape with a loop
        // in the first quadrant and asymptotic branches approaching
        // the line x + y + a = 0.
        let a: Float = 1.0
        ...
        for rs in 0..<revolutionSteps {
            ...
            for ps in 0..<profileSamples {
                // Parameter t from -0.8 to 8.0 (avoiding t = -1 where denominator = 0)
                let t = Float(ps) / Float(profileSamples) * 8.8 - 0.8
                let denom = 1.0 + t * t * t
                guard abs(denom) > 0.01 else { continue }
                let fx = 3.0 * a * t / denom
                let fy = 3.0 * a * t * t / denom
                let px = fx * scale / 2.0
                let py = fy * scale / 2.0 - half * 0.2
                // Revolve around Y axis
                let wx = px * cosPhi + half
                let wy = py + half
                let wz = px * sinPhi + half
    ...
    mutating func loadStrophoid() {
        ...
        // Right Strophoid: r = -a·cos(2θ)/cos(θ)
        // Parametric form: x = a(t² - 1)/(t² + 1), y = a·t(t² - 1)/(t² + 1)
        // The curve has a loop at the origin and an asymptote at x = a
        let a: Float = 1.0
        ...
        for rs in 0..<revolutionSteps {
            ...
            for ps in 0..<profileSamples {
                let t = Float(ps) / Float(profileSamples) * 4.0 - 2.0
                let t2 = t * t
                let denom = t2 + 1.0
                let px = a * (t2 - 1.0) / denom
                let py = a * t * (t2 - 1.0) / denom
                let sx = px * scale / (1.5 * a)
                let sy = py * scale / (2.0 * a)
                // Revolve around Y axis
                let wx = sx * cosPhi + half
                let wy = sy + half
                let wz = sx * sinPhi + half
    ...
    mutating func loadConchoid() {
        ...
        // Conchoid of Nicomedes: r = a + b·sec(θ)
        // In Cartesian: x = a·cos(θ) + b, y = a·sin(θ) + b·tan(θ)
        // Using a = 1.0, b = 0.5 for inner-loop variant
        let a: Float = 1.0
        let b: Float = 0.5
        ...
        for rs in 0..<revolutionSteps {
            ...
            for ps in 0..<profileSamples {
                let theta = Float(ps) / Float(profileSamples) * Float.pi * 1.6 - Float.pi * 0.8
                let cosTheta = cos(theta)
                guard abs(cosTheta) > 0.05 else { continue }
                let sinTheta = sin(theta)
                let r = a + b / cosTheta
                let px = r * cosTheta
                let py = r * sinTheta
