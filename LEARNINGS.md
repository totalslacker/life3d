...
- **`.once` timing expiry and restart pattern**: `emitter.timing = .once(...)` fires a single burst and then becomes inert — setting `isEmitting = true` again on a pooled emitter has no effect after the first fire. The fix is to re-assign `emitter.timing` to a fresh `.once(warmUp: 0, emit: VariableDuration(duration: X))` value immediately before each `isEmitting = true` at every trigger site. Because `ParticleEmitterComponent` is a Swift value type, this produces a new `timing` value with no "has-fired" state; RealityKit receives clean timing state when `entity.components.set(emitter)` is called. This avoids the unconfirmed `emitter.restart()` API (visionOS 2 behavior unclear) and avoids full component replacement (which would strip color state set by `updateParticleEmitterColors()`).
