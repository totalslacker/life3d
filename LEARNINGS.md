...
- **`.once` timing expiry and restart pattern (Option C)**: `emitter.timing = .once(...)` fires a single burst and then becomes inert. Re-assigning `emitter.timing` to a fresh `.once(...)` value (Option B) was found insufficient — RealityKit maintains internal "has-fired" state on the entity that is not reset by field re-assignment alone. The correct fix is full component replacement (Option C): call `makeParticleEmitterComponent(isBirth:themeColors:)` to construct a fresh `ParticleEmitterComponent` at each trigger site, then set `isEmitting = true` and `entity.components.set(emitter)`. A freshly constructed component has no firing history. Re-read `engine.theme` at trigger time to re-apply color — this also ensures color accuracy regardless of `updateParticleEmitterColors()` timing. See ADR 001.
