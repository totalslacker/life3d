...
- **`.once` timing expiry and restart pattern (Options B–E)**: `emitter.timing = .once(...)` fires a single burst and then becomes inert. Four options were attempted to restart it on pooled entities — all B/C/D failed because they all reuse the same entity object across burst cycles:
  - **Option B** (re-assign timing field): insufficient — entity-side "has-fired" state survives field re-assignment.
  - **Option C** (full fresh component via `components.set`): insufficient — `components.set` performs an in-place update when a component of that type already exists in the slot, preserving entity-side state.
  - **Option D** (remove-before-set): `components.remove()` then `components.set()` — confirmed via 164+ generations of diagnostic prints that the Swift path ran, but visual verification was never obtained. ADR 001 incorrectly marked this "Confirmed effective" based on prints alone.
  - **Root cause**: RealityKit retains particle-system VFX state at the **entity object level**. No component manipulation on a reused entity can clear it. The only fix is to allocate a fresh `Entity` for each burst.
  - **Option E (current)**: destroy-and-recreate. Each burst spawns a fresh `Entity`, attaches a fresh `ParticleEmitterComponent` via `makeParticleEmitterComponent()`, is parented to the container, and self-removes after its lifetime via `Task { entity.removeFromParent() }`. A newly constructed entity has no prior VFX history. **This is the only approach that works.** See ADR 001, Option E. `burstCount` is set by the caller at spawn time.
- **Per-kind entity caps prevent starvation (issue #15)**: With a shared cap, a sequential loop that spawns births first and deaths second will starve deaths if demand exceeds the cap. At 5 gen/s, 20 births × 6 overlapping generations = 120 concurrent birth-slot demand against a shared cap of 40 — cap saturates within 2–3 generations and deaths get zero slots. Fix: split into `activeBirthBurstCount` / `maxBirthBurstEntities = 20` and `activeDeathBurstCount` / `maxDeathBurstEntities = 20`. Each kind has its own hard budget; deaths can never be starved by births. Total ceiling stays at 40. The decrement in `Task { @MainActor in }` must use the same per-kind counter as the increment — Swift captures `isBirth: Bool` by value automatically, so no explicit capture list is needed.
