...
- **`.once` timing expiry and restart pattern (Option C)**: `emitter.timing = .once(...)` fires a single burst and then becomes inert. Re-assigning `emitter.timing` to a fresh `.once(...)` value (Option B) was found insufficient — RealityKit maintains internal "has-fired" state on the entity that is not reset by field re-assignment alone. The correct fix is full component replacement (Option C): call a dedicated `make*EmitterComponent()` helper to construct a fresh `ParticleEmitterComponent` at each trigger site, set `isEmitting = true`, then `entity.components.set(emitter)`. A freshly constructed component has no firing history. Re-read `engine.theme` at trigger time to re-apply color. This pattern applies to ALL emitter types in this project — birth/death (`makeParticleEmitterComponent`) and pulse (`makePulseEmitterComponent`). `burstCount` is NOT set inside the helper; the trigger site sets it explicitly so the trigger remains the sole source of truth. See ADR 001.
