...
struct ForestThemeTests {
    ...
    func themeCount() {
        #expect(ColorTheme.allThemes.count == 132)
    }
...
struct AliveMapResetRegressionTests {
    ...
    func patternCount() {
        ...
        #expect(allPatterns.count == 127)
        let cyclable = allPatterns.filter { $0 != .clear }
        #expect(cyclable.count == 127)
    }
}
...
struct AragoniteThemeTests {
    ...
    @Test func chrysoberylDistinctFromGoldSession135() {
...
struct ChrysoberylThemeTests {
    ...
    @Test func chrysoberylDistinctFromGoldSession136() {
        let chrysoberyl = ColorTheme.chrysoberyl
        let gold = ColorTheme.gold
        let rDiff = abs(chrysoberyl.newborn.baseColor.x - gold.newborn.baseColor.x)
        let gDiff = abs(chrysoberyl.newborn.baseColor.y - gold.newborn.baseColor.y)
...
struct TractrixPatternTests {
    @Test func tractrixProducesCells() {
        var grid = GridModel(size: 16)
        grid.loadTractrix()
        #expect(grid.aliveCount > 0)
    }
    @Test func tractrixIsCyclable() {
        let cyclable = SimulationEngine.Pattern.allCases.filter { $0 != .clear }
        #expect(cyclable.contains(.tractrix))
        #expect(cyclable.count == 127)
    }
    @Test func tractrixIsCentered() {
        var grid = GridModel(size: 16)
        grid.loadTractrix()
        var sumX = 0, sumY = 0, sumZ = 0, count = 0
        for x in 0..<16 { for y in 0..<16 { for z in 0..<16 {
    ...
    @Test func tractrixStaysInBounds() {
        var grid = GridModel(size: 16)
        grid.loadTractrix()
        #expect(grid.aliveCount <= 16 * 16 * 16)
    }
    @Test func tractrixScalesWithGridSize() {
        var small = GridModel(size: 12)
        small.loadTractrix()
        var large = GridModel(size: 24)
        large.loadTractrix()
    ...
    @Test func tractrixDistinctFromInvolute() {
        var tractGrid = GridModel(size: 16)
        tractGrid.loadTractrix()
        var invGrid = GridModel(size: 16)
        invGrid.loadInvolute()
        #expect(tractGrid.aliveCount != invGrid.aliveCount)
    }
    @Test func tractrixDistinctFromPseudosphere() {
        var tractGrid = GridModel(size: 16)
        tractGrid.loadTractrix()
        var pseudoGrid = GridModel(size: 16)
        pseudoGrid.loadPseudosphere()
        #expect(tractGrid.aliveCount != pseudoGrid.aliveCount)
    }
    @Test func tractrixDistinctFromCycloid() {
        var tractGrid = GridModel(size: 16)
        tractGrid.loadTractrix()
        var cycloidGrid = GridModel(size: 16)
        cycloidGrid.loadCycloid()
        #expect(tractGrid.aliveCount != cycloidGrid.aliveCount)
    }
    @Test func tractrixClearsBeforeLoading() {
        var grid = GridModel(size: 16)
        grid.randomSeed()
        let before = grid.aliveCount
        grid.loadTractrix()
    ...
    @Test func tractrixPatternCountUpdated() {
        let patterns = SimulationEngine.Pattern.allCases
        #expect(patterns.count == 128)
    }
}

// MARK: - Spectrolite Theme Tests

@Suite("Spectrolite Theme Tests")
struct SpectroliteThemeTests {
    @Test func spectroliteThemeExists() {
        let theme = ColorTheme.spectrolite
        #expect(theme.name == "Spectrolite")
    }
    @Test func spectroliteInAllThemes() {
        #expect(ColorTheme.allThemes.contains { $0.name == "Spectrolite" })
        #expect(ColorTheme.allThemes.count == 132)
    }
    @Test func spectroliteNewbornIsBlueDominant() {
        let theme = ColorTheme.spectrolite
        // B > G > R for vivid blue labradorescence
        #expect(theme.newborn.baseColor.z > theme.newborn.baseColor.y)
        #expect(theme.newborn.baseColor.y > theme.newborn.baseColor.x)
    }
    @Test func spectroliteOpacityDecreasesByAge() {
        let theme = ColorTheme.spectrolite
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func spectroliteDistinctFromLabradorite() {
        let spectrolite = ColorTheme.spectrolite
        let labradorite = ColorTheme.labradorite
        let rDiff = abs(spectrolite.newborn.baseColor.x - labradorite.newborn.baseColor.x)
        let gDiff = abs(spectrolite.newborn.baseColor.y - labradorite.newborn.baseColor.y)
        let bDiff = abs(spectrolite.newborn.baseColor.z - labradorite.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}
