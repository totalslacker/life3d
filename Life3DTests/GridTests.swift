...
struct ForestThemeTests {
    ...
    func themeCount() {
        #expect(ColorTheme.allThemes.count == 84)
    }
...
struct CeladonThemeTests {
    ...
    @Test func celadonOpacityDecreases() {
        let t = ColorTheme.celadon
        #expect(t.newborn.opacity > t.young.opacity)
        #expect(t.young.opacity > t.mature.opacity)
        #expect(t.mature.opacity > t.dying.opacity)
    ...
    @Test func pewterCoolMetallicGrey() {
        let nb = ColorTheme.pewter.newborn.baseColor
        // Pewter: B slightly dominant over G over R — cool metallic grey
        #expect(nb.z > nb.x)
        // All channels relatively close (grey, not saturated)
        #expect(abs(nb.x - nb.z) < 0.15)
    }
...
// MARK: - Ellipsoid Pattern Tests

@Suite("Ellipsoid Pattern Tests")
struct EllipsoidPatternTests {
    @Test func ellipsoidProducesCells() {
        var grid = GridModel(size: 16)
        grid.loadEllipsoid()
        #expect(grid.aliveCount > 50)
    }

    @Test func ellipsoidCentered() {
        var grid = GridModel(size: 16)
        grid.loadEllipsoid()
        var sumX = 0, sumY = 0, sumZ = 0, count = 0
        for idx in grid.aliveCellIndices {
    ...
    @Test func ellipsoidDistinctFromSphere() {
        var ellipsoid = GridModel(size: 16)
        ellipsoid.loadEllipsoid()
        var sphere = GridModel(size: 16)
        sphere.loadSphere()
        #expect(ellipsoid.aliveCount != sphere.aliveCount)
    }

    @Test func ellipsoidDistinctFromFermatSurface() {
        var ellipsoid = GridModel(size: 16)
        ellipsoid.loadEllipsoid()
        var fermat = GridModel(size: 16)
        fermat.loadFermatSurface()
        #expect(ellipsoid.aliveCount != fermat.aliveCount)
    }

    @Test func ellipsoidSurvivesEvolution() {
        var grid = GridModel(size: 16, birthCounts: [5, 6, 7], survivalCounts: [5, 6, 7, 8])
        grid.loadEllipsoid()
        let initial = grid.aliveCount
        grid.advanceGeneration()
    ...
    @Test func ellipsoidScalesWithGridSize() {
        var small = GridModel(size: 8)
        small.loadEllipsoid()
        var large = GridModel(size: 16)
        large.loadEllipsoid()
...
struct FluoriteThemeTests {
    @Test func fluoritePurpleDominant() {
        ...
        // Fluorite newborn: vivid purple, B > R > G
        #expect(nb.z > nb.x)  // B > R
        #expect(nb.x > nb.y)  // R > G
    }

    @Test func fluoriteShiftsToCoolGreen() {
        let young = ColorTheme.fluorite.young.baseColor
        // Young tier shifts to green: G > B > R (mimics fluorite banding)
        #expect(young.y > young.x)  // G > R
    }
    ...
    @Test func fluoriteDistinctFromAlexandrite() {
        let fluorite = ColorTheme.fluorite.newborn.baseColor
        let alexandrite = ColorTheme.alexandrite.newborn.baseColor
        let diff = abs(fluorite.x - alexandrite.x) + abs(fluorite.y - alexandrite.y) + abs(fluorite.z - alexandrite.z)
        #expect(diff > 0.1)
    }
