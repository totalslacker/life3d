...
// MARK: - Grandidierite Theme Tests

@Suite("Grandidierite Theme Tests")
struct GrandidieriteThemeTests {
    @Test func grandidieriteThemeExists() {
        let theme = ColorTheme.grandidierite
        #expect(theme.name == "Grandidierite")
    }
    @Test func grandidieriteThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Grandidierite" })
    }
    @Test func grandidieriteThemeBlueGreenDominant() {
        let theme = ColorTheme.grandidierite
        // G > B > R for blue-green
        #expect(theme.newborn.baseColor.y > theme.newborn.baseColor.z)
        #expect(theme.newborn.baseColor.z > theme.newborn.baseColor.x)
    }
    @Test func grandidieriteThemeOpacityDecreases() {
        let theme = ColorTheme.grandidierite
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func grandidieriteDistinctFromCelestite() {
        let grandidierite = ColorTheme.grandidierite
        let celestite = ColorTheme.celestite
        let rDiff = abs(grandidierite.newborn.baseColor.x - celestite.newborn.baseColor.x)
        let gDiff = abs(grandidierite.newborn.baseColor.y - celestite.newborn.baseColor.y)
        let bDiff = abs(grandidierite.newborn.baseColor.z - celestite.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}

// MARK: - Strontianite Theme Tests

@Suite("Strontianite Theme Tests")
struct StrontianiteThemeTests {
    @Test func strontianiteThemeExists() {
        let theme = ColorTheme.strontianite
        #expect(theme.name == "Strontianite")
    }
    @Test func strontianiteThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Strontianite" })
    }
    @Test func strontianiteThemeYellowGreenDominant() {
        let theme = ColorTheme.strontianite
        // G >= R > B for yellow-green
        #expect(theme.newborn.baseColor.y >= theme.newborn.baseColor.x - 0.05)
        #expect(theme.newborn.baseColor.x > theme.newborn.baseColor.z)
    }
    @Test func strontianiteThemeOpacityDecreases() {
        let theme = ColorTheme.strontianite
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func strontianiteDistinctFromChartreuse() {
        let strontianite = ColorTheme.strontianite
        let chartreuse = ColorTheme.chartreuse
        let rDiff = abs(strontianite.newborn.baseColor.x - chartreuse.newborn.baseColor.x)
        let gDiff = abs(strontianite.newborn.baseColor.y - chartreuse.newborn.baseColor.y)
        let bDiff = abs(strontianite.newborn.baseColor.z - chartreuse.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}

// MARK: - Wulfenite Theme Tests

@Suite("Wulfenite Theme Tests")
struct WulfeniteThemeTests {
    @Test func wulfeniteThemeExists() {
        let theme = ColorTheme.wulfenite
        #expect(theme.name == "Wulfenite")
    }
    @Test func wulfeniteThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Wulfenite" })
    }
    @Test func wulfeniteThemeOrangeYellowDominant() {
        let theme = ColorTheme.wulfenite
        // R > G > B for orange-yellow lead molybdate
        #expect(theme.newborn.baseColor.x > theme.newborn.baseColor.y)
        #expect(theme.newborn.baseColor.y > theme.newborn.baseColor.z)
    }
    @Test func wulfeniteThemeOpacityDecreases() {
        let theme = ColorTheme.wulfenite
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func wulfeniteDistinctFromSphalerite() {
        let wulfenite = ColorTheme.wulfenite
        let sphalerite = ColorTheme.sphalerite
        let rDiff = abs(wulfenite.newborn.baseColor.x - sphalerite.newborn.baseColor.x)
        let gDiff = abs(wulfenite.newborn.baseColor.y - sphalerite.newborn.baseColor.y)
        let bDiff = abs(wulfenite.newborn.baseColor.z - sphalerite.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}

// MARK: - Strophoid Pattern Tests

@Suite("Strophoid Pattern Tests")
struct StrophoidPatternTests {
    @Test func strophoidPatternExists() {
        let pattern = SimulationEngine.Pattern.strophoid
        #expect(pattern.rawValue == "Strophoid")
    }
    @Test func strophoidPatternIsCyclable() {
        let allPatterns = SimulationEngine.Pattern.allCases
        let cyclable = allPatterns.filter { $0 != .clear }
        #expect(cyclable.contains(.strophoid))
    }
    @Test func strophoidPatternLoadsCells() {
        var grid = GridModel(size: 16)
        grid.loadStrophoid()
        #expect(grid.aliveCount > 0)
    }
    @Test func strophoidPatternSymmetry() {
        var grid = GridModel(size: 16)
        grid.loadStrophoid()
        #expect(grid.aliveCount > 50)
    }
    @Test func strophoidPatternClearsBeforeLoad() {
        var grid = GridModel(size: 16)
        grid.randomSeed()
        let beforeCount = grid.aliveCount
        grid.loadStrophoid()
    ...
    @Test func strophoidPatternDifferentSizes() {
        var grid8 = GridModel(size: 8)
        grid8.loadStrophoid()
        var grid16 = GridModel(size: 16)
        grid16.loadStrophoid()
        #expect(grid16.aliveCount > grid8.aliveCount)
    }
    @Test func strophoidPatternDistinctFromCissoid() {
        var gridA = GridModel(size: 16)
        gridA.loadStrophoid()
        var gridB = GridModel(size: 16)
        gridB.loadCissoid()
    ...
    @Test func strophoidPatternDistinctFromFoliumOfDescartes() {
        var gridA = GridModel(size: 16)
        gridA.loadStrophoid()
        var gridB = GridModel(size: 16)
        gridB.loadFoliumOfDescartes()
        #expect(gridA.aliveCount != gridB.aliveCount)
    }
    @Test func strophoidPatternAliveIndicesConsistent() {
        var grid = GridModel(size: 16)
        grid.loadStrophoid()
        #expect(grid.aliveCellIndices.count == grid.aliveCount)
    }
    @Test func strophoidPatternInAllCases() {
        let allPatterns = SimulationEngine.Pattern.allCases
        #expect(allPatterns.count == 139)
        #expect(allPatterns.contains(.strophoid))
    }
}

// MARK: - Vivianite Theme Tests

@Suite("Vivianite Theme Tests")
struct VivianiteThemeTests {
    @Test func vivianiteThemeExists() {
        let theme = ColorTheme.vivianite
        #expect(theme.name == "Vivianite")
    }
    @Test func vivianiteThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Vivianite" })
    }
    @Test func vivianiteThemeBlueDominant() {
        let theme = ColorTheme.vivianite
        // B > G > R for deep blue-green
        #expect(theme.newborn.baseColor.z > theme.newborn.baseColor.y)
        #expect(theme.newborn.baseColor.y > theme.newborn.baseColor.x)
    }
    @Test func vivianiteThemeOpacityDecreases() {
        let theme = ColorTheme.vivianite
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func vivianiteDistinctFromAzurite() {
        let vivianite = ColorTheme.vivianite
        let azurite = ColorTheme.azurite
        let rDiff = abs(vivianite.newborn.baseColor.x - azurite.newborn.baseColor.x)
        let gDiff = abs(vivianite.newborn.baseColor.y - azurite.newborn.baseColor.y)
        let bDiff = abs(vivianite.newborn.baseColor.z - azurite.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}

// MARK: - Conchoid of Nicomedes Pattern Tests

@Suite("Conchoid of Nicomedes Pattern Tests")
struct ConchoidPatternTests {
    @Test func conchoidPatternExists() {
        let pattern = SimulationEngine.Pattern.conchoid
        #expect(pattern.rawValue == "Conchoid of Nicomedes")
    }
    @Test func conchoidPatternIsCyclable() {
        let allPatterns = SimulationEngine.Pattern.allCases
        let cyclable = allPatterns.filter { $0 != .clear }
        #expect(cyclable.contains(.conchoid))
    }
    @Test func conchoidPatternLoadsCells() {
        var grid = GridModel(size: 16)
        grid.loadConchoid()
        #expect(grid.aliveCount > 0)
    }
    @Test func conchoidPatternSymmetry() {
        var grid = GridModel(size: 16)
        grid.loadConchoid()
        #expect(grid.aliveCount > 50)
    }
    @Test func conchoidPatternClearsBeforeLoad() {
        var grid = GridModel(size: 16)
        grid.randomSeed()
        let beforeCount = grid.aliveCount
        grid.loadConchoid()
    ...
    @Test func conchoidPatternDifferentSizes() {
        var grid8 = GridModel(size: 8)
        grid8.loadConchoid()
        var grid16 = GridModel(size: 16)
        grid16.loadConchoid()
    ...
    @Test func conchoidPatternDistinctFromCissoid() {
        var gridA = GridModel(size: 16)
        gridA.loadConchoid()
        var gridB = GridModel(size: 16)
        gridB.loadCissoid()
    ...
    @Test func conchoidPatternDistinctFromWitchOfAgnesi() {
        var gridA = GridModel(size: 16)
        gridA.loadConchoid()
        var gridB = GridModel(size: 16)
        gridB.loadWitchOfAgnesi()
    ...
    @Test func conchoidPatternAliveIndicesConsistent() {
        var grid = GridModel(size: 16)
        grid.loadConchoid()
        #expect(grid.aliveCellIndices.count == grid.aliveCount)
    }
    @Test func conchoidPatternInAllCases() {
        let allPatterns = SimulationEngine.Pattern.allCases
        #expect(allPatterns.count == 139)
        #expect(allPatterns.contains(.conchoid))
    }
}

// MARK: - Piemontite Theme Tests

@Suite("Piemontite Theme Tests")
struct PiemontiteThemeTests {
    @Test func piemontiteThemeExists() {
        let theme = ColorTheme.piemontite
        #expect(theme.name == "Piemontite")
    }
    @Test func piemontiteThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Piemontite" })
    }
    @Test func piemontiteThemeRedVioletDominant() {
        let theme = ColorTheme.piemontite
        // R > B > G for reddish-violet
        #expect(theme.newborn.baseColor.x > theme.newborn.baseColor.z)
        #expect(theme.newborn.baseColor.z > theme.newborn.baseColor.y)
    }
    @Test func piemontiteThemeOpacityDecreases() {
        let theme = ColorTheme.piemontite
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func piemontiteDistinctFromRhodolite() {
        let piemontite = ColorTheme.piemontite
        let rhodolite = ColorTheme.rhodolite
        let rDiff = abs(piemontite.newborn.baseColor.x - rhodolite.newborn.baseColor.x)
        let gDiff = abs(piemontite.newborn.baseColor.y - rhodolite.newborn.baseColor.y)
        let bDiff = abs(piemontite.newborn.baseColor.z - rhodolite.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}

// MARK: - Scapolite Theme Tests

@Suite("Scapolite Theme Tests")
struct ScapoliteThemeTests {
    @Test func scapoliteThemeExists() {
        let theme = ColorTheme.scapolite
        #expect(theme.name == "Scapolite")
    }
    @Test func scapoliteThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Scapolite" })
    }
    @Test func scapoliteThemeYellowDominant() {
        let theme = ColorTheme.scapolite
        // R > G > B for golden-yellow
        #expect(theme.newborn.baseColor.x > theme.newborn.baseColor.y)
        #expect(theme.newborn.baseColor.y > theme.newborn.baseColor.z)
    }
    @Test func scapoliteThemeOpacityDecreases() {
        let theme = ColorTheme.scapolite
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func scapoliteDistinctFromCitrine() {
        let scapolite = ColorTheme.scapolite
        let citrine = ColorTheme.citrine
        let rDiff = abs(scapolite.newborn.baseColor.x - citrine.newborn.baseColor.x)
        let gDiff = abs(scapolite.newborn.baseColor.y - citrine.newborn.baseColor.y)
        let bDiff = abs(scapolite.newborn.baseColor.z - citrine.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}

// MARK: - Epidote Theme Tests

@Suite("Epidote Theme Tests")
struct EpidoteThemeTests {
    @Test func epidoteThemeExists() {
        let theme = ColorTheme.epidote
        #expect(theme.name == "Epidote")
    }
    @Test func epidoteThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Epidote" })
    }
    @Test func epidoteThemeGreenDominant() {
        let theme = ColorTheme.epidote
        // G > R > B for pistachio-green
        #expect(theme.newborn.baseColor.y > theme.newborn.baseColor.x)
        #expect(theme.newborn.baseColor.x > theme.newborn.baseColor.z)
    }
    @Test func epidoteThemeOpacityDecreases() {
        let theme = ColorTheme.epidote
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func epidoteDistinctFromVariscite() {
        let epidote = ColorTheme.epidote
        let variscite = ColorTheme.variscite
        let rDiff = abs(epidote.newborn.baseColor.x - variscite.newborn.baseColor.x)
        let gDiff = abs(epidote.newborn.baseColor.y - variscite.newborn.baseColor.y)
        let bDiff = abs(epidote.newborn.baseColor.z - variscite.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}

// MARK: - Smithsonite Theme Tests

@Suite("Smithsonite Theme Tests")
struct SmithsoniteThemeTests {
    @Test func smithsoniteThemeExists() {
        let theme = ColorTheme.smithsonite
        #expect(theme.name == "Smithsonite")
    }
    @Test func smithsoniteThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Smithsonite" })
    }
    @Test func smithsoniteThemeBlueTealDominant() {
        let theme = ColorTheme.smithsonite
        // Smithsonite: pastel blue-teal — B > G > R
        #expect(theme.newborn.baseColor.z > theme.newborn.baseColor.y)
        #expect(theme.newborn.baseColor.y > theme.newborn.baseColor.x)
    }
    @Test func smithsoniteThemeOpacityDecreases() {
        let theme = ColorTheme.smithsonite
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func smithsoniteDistinctFromCelestite() {
        let smithsonite = ColorTheme.smithsonite
        let celestite = ColorTheme.celestite
        let rDiff = abs(smithsonite.newborn.baseColor.x - celestite.newborn.baseColor.x)
        let gDiff = abs(smithsonite.newborn.baseColor.y - celestite.newborn.baseColor.y)
        let bDiff = abs(smithsonite.newborn.baseColor.z - celestite.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}

// MARK: - Trident of Newton Pattern Tests

@Suite("Trident of Newton Pattern Tests")
struct TridentOfNewtonPatternTests {
    @Test func tridentPatternExists() {
        let pattern = SimulationEngine.Pattern.tridentOfNewton
        #expect(pattern.rawValue == "Trident of Newton")
    }
    @Test func tridentPatternIsCyclable() {
        let allPatterns = SimulationEngine.Pattern.allCases
        let cyclable = allPatterns.filter { $0 != .clear }
        #expect(cyclable.contains(.tridentOfNewton))
    }
    @Test func tridentPatternLoadsCells() {
        var grid = GridModel(size: 16)
        grid.loadTridentOfNewton()
        #expect(grid.aliveCount > 0)
    }
    @Test func tridentPatternSubstantialCells() {
        var grid = GridModel(size: 16)
        grid.loadTridentOfNewton()
        #expect(grid.aliveCount > 50)
    }
    @Test func tridentPatternClearsBeforeLoad() {
        var grid = GridModel(size: 16)
        grid.randomSeed()
        let beforeCount = grid.aliveCount
        grid.loadTridentOfNewton()
    ...
    @Test func tridentPatternDifferentSizes() {
        var grid8 = GridModel(size: 8)
        grid8.loadTridentOfNewton()
        var grid16 = GridModel(size: 16)
        grid16.loadTridentOfNewton()
        #expect(grid16.aliveCount > grid8.aliveCount)
    }
    @Test func tridentPatternDistinctFromConchoid() {
        var gridA = GridModel(size: 16)
        gridA.loadTridentOfNewton()
        var gridB = GridModel(size: 16)
        gridB.loadConchoid()
        #expect(gridA.aliveCount != gridB.aliveCount)
    }
    @Test func tridentPatternDistinctFromStrophoid() {
        var gridA = GridModel(size: 16)
        gridA.loadTridentOfNewton()
        var gridB = GridModel(size: 16)
        gridB.loadStrophoid()
    ...
    @Test func tridentPatternAliveIndicesConsistent() {
        var grid = GridModel(size: 16)
        grid.loadTridentOfNewton()
        #expect(grid.aliveCellIndices.count == grid.aliveCount)
    }
    @Test func tridentPatternInAllCases() {
        let allPatterns = SimulationEngine.Pattern.allCases
        #expect(allPatterns.count == 139)
        #expect(allPatterns.contains(.tridentOfNewton))
    }
}

// MARK: - Zoisite Theme Tests

@Suite("Zoisite Theme Tests")
struct ZoisiteThemeTests {
    @Test func zoisiteThemeExists() {
        let theme = ColorTheme.zoisite
        #expect(theme.name == "Zoisite")
    }
    @Test func zoisiteThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Zoisite" })
    }
    @Test func zoisiteThemeBluePurpleDominant() {
        let theme = ColorTheme.zoisite
        // B > R > G for blue-violet
        #expect(theme.newborn.baseColor.z > theme.newborn.baseColor.x)
        #expect(theme.newborn.baseColor.x > theme.newborn.baseColor.y)
    }
    @Test func zoisiteThemeOpacityDecreases() {
        let theme = ColorTheme.zoisite
// MARK: - Tractrix Pattern Tests

@Suite("Tractrix Pattern Tests")
struct TractrixPatternTests {
    @Test func tractrixPatternProducesCells() {
        var grid = GridModel(size: 16)
        grid.loadTractrix()
        #expect(grid.aliveCount > 0)
    }
    @Test func tractrixPatternNotTooFull() {
        var grid = GridModel(size: 16)
        grid.loadTractrix()
        #expect(grid.aliveCount < grid.cellCount)
    }
    @Test func tractrixPatternDeterministic() {
        var g1 = GridModel(size: 16)
        g1.loadTractrix()
        var g2 = GridModel(size: 16)
        g2.loadTractrix()
    ...
    @Test func tractrixPatternClearsFirst() {
        var grid = GridModel(size: 16)
    ...
    @Test func tractrixPatternScalesWithSize() {
        var small = GridModel(size: 8)
        small.loadTractrix()
        var large = GridModel(size: 16)
        large.loadTractrix()
    ...
    @Test func tractrixPatternStaysInBounds() {
    ...
    @Test func tractrixPatternAliveIndicesConsistent() {
        var grid = GridModel(size: 16)
        grid.loadTractrix()
        #expect(grid.aliveCellIndices.count == grid.aliveCount)
    }
    @Test func tractrixPatternInAllCases() {
        let allPatterns = SimulationEngine.Pattern.allCases
        #expect(allPatterns.count == 139)
        #expect(allPatterns.contains(.tractrix))
    }
    @Test func tractrixPatternCyclable() {
        let cyclable = SimulationEngine.Pattern.allCases.filter { $0 != .clear }
        #expect(cyclable.count == 138)
        #expect(cyclable.contains(.tractrix))
    }
    @Test func tractrixPatternSymmetric() {
        var grid = GridModel(size: 16)
        grid.loadTractrix()
        // Revolved around X axis, should have approximate Y-Z symmetry
        let mid = grid.size / 2
        var hasUpperY = false
        var hasLowerY = false
        for idx in grid.aliveCellIndices {
            let y = (idx / grid.size) % grid.size
            if y > mid { hasUpperY = true }
            if y < mid { hasLowerY = true }
        }
        #expect(hasUpperY)
        #expect(hasLowerY)
    }
...
// MARK: - Kornerupine Theme Tests

@Suite("Kornerupine Theme Tests")
struct KornerupineThemeTests {
    @Test func kornerupineThemeExists() {
        let theme = ColorTheme.kornerupine
        #expect(theme.name == "Kornerupine")
    }
    @Test func kornerupineThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Kornerupine" })
    }
    @Test func kornerupineThemeGreenDominant() {
        let theme = ColorTheme.kornerupine
        // G > R > B for olive-green
        #expect(theme.newborn.baseColor.y > theme.newborn.baseColor.x)
        #expect(theme.newborn.baseColor.x > theme.newborn.baseColor.z)
    }
    @Test func kornerupineThemeOpacityDecreases() {
        let theme = ColorTheme.kornerupine
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func zoisiteDistinctFromTanzanite() {
        let zoisite = ColorTheme.zoisite
        let tanzanite = ColorTheme.tanzanite
        let rDiff = abs(zoisite.newborn.baseColor.x - tanzanite.newborn.baseColor.x)
        let gDiff = abs(zoisite.newborn.baseColor.y - tanzanite.newborn.baseColor.y)
        let bDiff = abs(zoisite.newborn.baseColor.z - tanzanite.newborn.baseColor.z)
    @Test func kornerupineDistinctFromPrehnite() {
        let kornerupine = ColorTheme.kornerupine
        let prehnite = ColorTheme.prehnite
        let rDiff = abs(kornerupine.newborn.baseColor.x - prehnite.newborn.baseColor.x)
        let gDiff = abs(kornerupine.newborn.baseColor.y - prehnite.newborn.baseColor.y)
        let bDiff = abs(kornerupine.newborn.baseColor.z - prehnite.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}
...
// MARK: - Kampyle of Eudoxus Pattern Tests

@Suite("Kampyle Pattern Tests")
struct KampylePatternTests {
    @Test func kampylePatternProducesCells() {
        var grid = GridModel(size: 16)
        grid.loadKampyle()
        #expect(grid.aliveCount > 0)
    }
    @Test func kampylePatternNotTooFull() {
        var grid = GridModel(size: 16)
        grid.loadKampyle()
        #expect(grid.aliveCount < grid.cellCount)
    }
    @Test func kampylePatternDeterministic() {
        var g1 = GridModel(size: 16)
        g1.loadKampyle()
        var g2 = GridModel(size: 16)
        g2.loadKampyle()
    ...
    @Test func kampylePatternClearsFirst() {
        var grid = GridModel(size: 16)
        grid.randomSeed(density: 1.0)
        let fullCount = grid.aliveCount
        grid.loadKampyle()
    ...
    @Test func kampylePatternScalesWithSize() {
        var small = GridModel(size: 8)
        small.loadKampyle()
        var large = GridModel(size: 16)
        large.loadKampyle()
    ...
    @Test func kampylePatternStaysInBounds() {
        var grid = GridModel(size: 12)
        grid.loadKampyle()
        for idx in grid.aliveCellIndices {
            #expect(idx >= 0 && idx < grid.cellCount)
    ...
    @Test func kampylePatternAliveIndicesConsistent() {
        var grid = GridModel(size: 16)
        grid.loadKampyle()
        #expect(grid.aliveCellIndices.count == grid.aliveCount)
    }
    @Test func kampylePatternInAllCases() {
        let allPatterns = SimulationEngine.Pattern.allCases
        #expect(allPatterns.count == 139)
        #expect(allPatterns.contains(.kampyle))
    }
    @Test func kampylePatternCyclable() {
        let cyclable = SimulationEngine.Pattern.allCases.filter { $0 != .clear }
        #expect(cyclable.count == 138)
        #expect(cyclable.contains(.kampyle))
    }
    @Test func kampylePatternSymmetric() {
        var grid = GridModel(size: 16)
        grid.loadKampyle()
        let mid = grid.size / 2
        var aboveCount = 0
        ...
        for idx in grid.aliveCellIndices {
            let y = (idx / grid.size) % grid.size
            if y > mid { aboveCount += 1 }
            if y < mid { belowCount += 1 }
        }
        #expect(aboveCount > 0)
        #expect(belowCount > 0)
    }
...
// MARK: - Sphene Theme Tests

@Suite("Sphene Theme Tests")
struct SpheneThemeTests {
    @Test func spheneThemeExists() {
        let theme = ColorTheme.sphene
        #expect(theme.name == "Sphene")
    }
    @Test func spheneThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Sphene" })
    }
    @Test func spheneThemeYellowGreen() {
        let theme = ColorTheme.sphene
        // G > R > B for yellow-green
        #expect(theme.newborn.baseColor.y > theme.newborn.baseColor.x)
        #expect(theme.newborn.baseColor.x > theme.newborn.baseColor.z)
    }
    @Test func spheneThemeOpacityDecreases() {
        let theme = ColorTheme.sphene
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func spheneDistinctFromChartreuse() {
        let sphene = ColorTheme.sphene
        let chartreuse = ColorTheme.chartreuse
        let rDiff = abs(sphene.newborn.baseColor.x - chartreuse.newborn.baseColor.x)
        let gDiff = abs(sphene.newborn.baseColor.y - chartreuse.newborn.baseColor.y)
        let bDiff = abs(sphene.newborn.baseColor.z - chartreuse.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}

// MARK: - Cochleoid Pattern Tests

@Suite("Cochleoid Pattern Tests")
struct CochleoidPatternTests {
    @Test func cochleoidPatternProducesCells() {
        var grid = GridModel(size: 16)
        grid.loadCochleoid()
        #expect(grid.aliveCount > 0)
    }
    @Test func cochleoidPatternNotTooFull() {
        var grid = GridModel(size: 16)
        grid.loadCochleoid()
        #expect(grid.aliveCount < grid.cellCount)
    }
    @Test func cochleoidPatternDeterministic() {
        var g1 = GridModel(size: 16)
        g1.loadCochleoid()
        var g2 = GridModel(size: 16)
        g2.loadCochleoid()
    ...
    @Test func cochleoidPatternClearsFirst() {
        var grid = GridModel(size: 16)
        for x in 0..<grid.size { for y in 0..<grid.size { for z in 0..<grid.size {
            grid.setCell(x: x, y: y, z: z, alive: true)
        }}}
        grid.loadCochleoid()
        #expect(grid.aliveCount < grid.cellCount)
    }
    @Test func cochleoidPatternScalesWithSize() {
        var small = GridModel(size: 8)
        small.loadCochleoid()
        var large = GridModel(size: 16)
        large.loadCochleoid()
    ...
    @Test func cochleoidPatternStaysInBounds() {
        var grid = GridModel(size: 12)
        grid.loadCochleoid()
        for idx in grid.aliveCellIndices {
            #expect(idx >= 0 && idx < grid.cellCount)
    ...
    @Test func cochleoidPatternAliveIndicesConsistent() {
        var grid = GridModel(size: 16)
        grid.loadCochleoid()
        #expect(grid.aliveCellIndices.count == grid.aliveCount)
    }
    @Test func cochleoidPatternInAllCases() {
        let allPatterns = SimulationEngine.Pattern.allCases
        #expect(allPatterns.count == 139)
        #expect(allPatterns.contains(.cochleoid))
    }
    @Test func cochleoidPatternCyclable() {
        let cyclable = SimulationEngine.Pattern.allCases.filter { $0 != .clear }
        #expect(cyclable.count == 138)
        #expect(cyclable.contains(.cochleoid))
    }
    @Test func cochleoidPatternSpiralShape() {
        var grid = GridModel(size: 16)
        grid.loadCochleoid()
        let mid = grid.size / 2
        // Cochleoid spirals around origin — expect cells in multiple quadrants
        var q1 = 0; var q2 = 0; var q3 = 0; var q4 = 0
        for idx in grid.aliveCellIndices {
            let x = idx / (grid.size * grid.size)
            let y = (idx / grid.size) % grid.size
            if x >= mid && y >= mid { q1 += 1 }
            if x < mid && y >= mid { q2 += 1 }
            if x < mid && y < mid { q3 += 1 }
...
// MARK: - Crocoite Theme Tests

@Suite("Crocoite Theme Tests")
struct CrocoiteThemeTests {
    @Test func crocoiteThemeExists() {
        let theme = ColorTheme.crocoite
        #expect(theme.name == "Crocoite")
    }
    @Test func crocoiteThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Crocoite" })
    }
    @Test func crocoiteThemeOrangeRed() {
        let theme = ColorTheme.crocoite
        // R > G > B for orange-red
        #expect(theme.newborn.baseColor.x > theme.newborn.baseColor.y)
        #expect(theme.newborn.baseColor.y > theme.newborn.baseColor.z)
    }
    @Test func crocoiteThemeOpacityDecreases() {
        let theme = ColorTheme.crocoite
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func crocoiteDistinctFromVermilion() {
        let crocoite = ColorTheme.crocoite
        let vermilion = ColorTheme.vermilion
        let rDiff = abs(crocoite.newborn.baseColor.x - vermilion.newborn.baseColor.x)
        let gDiff = abs(crocoite.newborn.baseColor.y - vermilion.newborn.baseColor.y)
        let bDiff = abs(crocoite.newborn.baseColor.z - vermilion.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}

// MARK: - Cornu Spiral Pattern Tests

@Suite("Cornu Spiral Pattern Tests")
struct CornuSpiralPatternTests {
    @Test func cornuSpiralPatternProducesCells() {
        var grid = GridModel(size: 16)
        grid.loadCornuSpiral()
        #expect(grid.aliveCount > 0)
    }
    @Test func cornuSpiralPatternAliveIndicesConsistent() {
        var grid = GridModel(size: 16)
        grid.loadCornuSpiral()
        #expect(grid.aliveCellIndices.count == grid.aliveCount)
    }
    @Test func cornuSpiralPatternInAllCases() {
        let allPatterns = SimulationEngine.Pattern.allCases
        #expect(allPatterns.count == 139)
        #expect(allPatterns.contains(.cornuSpiral))
    }
    @Test func cornuSpiralPatternCyclable() {
        let cyclable = SimulationEngine.Pattern.allCases.filter { $0 != .clear }
        #expect(cyclable.count == 138)
        #expect(cyclable.contains(.cornuSpiral))
    }
    @Test func cornuSpiralPatternSShape() {
        var grid = GridModel(size: 16)
        grid.loadCornuSpiral()
        let mid = grid.size / 2
        // Cornu spiral has an S-shape — cells in multiple quadrants
        var q1 = 0; var q2 = 0; var q3 = 0; var q4 = 0
        for idx in grid.aliveCellIndices {
            let x = idx / (grid.size * grid.size)
            let y = (idx / grid.size) % grid.size
    ...
    @Test func cornuSpiralPatternMultipleLayers() {
        var grid = GridModel(size: 16)
        grid.loadCornuSpiral()
        var layersOccupied = Set<Int>()
        for idx in grid.aliveCellIndices {
            let z = idx % grid.size
            layersOccupied.insert(z)
        }
        #expect(layersOccupied.count >= 3)
    }
    @Test func cornuSpiralPatternCenteredInGrid() {
        var grid = GridModel(size: 16)
        grid.loadCornuSpiral()
        let half = grid.size / 2
        // Center region should have cells (spiral passes through origin)
        var centerCount = 0
        for idx in grid.aliveCellIndices {
            let x = idx / (grid.size * grid.size)
            let y = (idx / grid.size) % grid.size
            if abs(x - half) <= 2 && abs(y - half) <= 2 { centerCount += 1 }
        }
        #expect(centerCount > 0)
    }
    @Test func cornuSpiralDifferentSizes() {
        var grid12 = GridModel(size: 12)
        grid12.loadCornuSpiral()
        var grid16 = GridModel(size: 16)
        grid16.loadCornuSpiral()
        #expect(grid12.aliveCount > 0)
        #expect(grid16.aliveCount > grid12.aliveCount)
    }
    @Test func cornuSpiralDistinctFromCochleoid() {
        var gridCornu = GridModel(size: 16)
        gridCornu.loadCornuSpiral()
        var gridCochleoid = GridModel(size: 16)
        gridCochleoid.loadCochleoid()
        // Different patterns should produce different cell counts
        #expect(gridCornu.aliveCount != gridCochleoid.aliveCount)
    }
    @Test func cornuSpiralSurvivesGeneration() {
        var grid = GridModel(size: 16)
        grid.loadCornuSpiral()
        let initial = grid.aliveCount
        grid.advanceGeneration()
        // Pattern should evolve (not be static)
        #expect(grid.aliveCount != initial || grid.aliveCount > 0)
    }
}

// MARK: - Ametrine Theme Tests

@Suite("Ametrine Theme Tests")
struct AmetrineThemeTests {
    @Test func ametrineThemeExists() {
        let theme = ColorTheme.ametrine
        #expect(theme.name == "Ametrine")
    }
    @Test func ametrineThemeInAllThemes() {
        #expect(ColorTheme.allThemes.count == 152)
        #expect(ColorTheme.allThemes.contains { $0.name == "Ametrine" })
    }
    @Test func ametrineThemePurpleNewborn() {
        let theme = ColorTheme.ametrine
        // Newborn is purple: B > G, R > G
        #expect(theme.newborn.baseColor.z > theme.newborn.baseColor.y)
        #expect(theme.newborn.baseColor.x > theme.newborn.baseColor.y)
    }
    @Test func ametrineThemeOpacityDecreases() {
        let theme = ColorTheme.ametrine
        #expect(theme.newborn.opacity > theme.young.opacity)
        #expect(theme.young.opacity > theme.mature.opacity)
        #expect(theme.mature.opacity > theme.dying.opacity)
    }
    @Test func ametrineDistinctFromAmethyst() {
        let ametrine = ColorTheme.ametrine
        let amethyst = ColorTheme.amethyst
        let rDiff = abs(ametrine.newborn.baseColor.x - amethyst.newborn.baseColor.x)
        let gDiff = abs(ametrine.newborn.baseColor.y - amethyst.newborn.baseColor.y)
        let bDiff = abs(ametrine.newborn.baseColor.z - amethyst.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}
...
struct BorniteThemeTests {
    ...
    @Test func danburiteDistinctFromChampagne() {
        let danburite = ColorTheme.danburite
        let champagne = ColorTheme.champagne
        let rDiff = abs(danburite.newborn.baseColor.x - champagne.newborn.baseColor.x)
        let gDiff = abs(danburite.newborn.baseColor.y - champagne.newborn.baseColor.y)
        let bDiff = abs(danburite.newborn.baseColor.z - champagne.newborn.baseColor.z)
        #expect(rDiff + gDiff + bDiff > 0.05)
    }
}
